...
SYSTEM_PROMPT_TEMPLATE = """你是专业的奢侈品和时尚商品鉴定专家，擅长通过图片识别服装品牌、型号、材质和价格。
...
6. 根据图片内容，从提供的 POSHMARK_CATEGORIES 中选择最合适的分类路径

输出必须严格遵循以下 JSON 格式（注意：所有 JSON 键值对必须用双引号，数字不需要引号）：
{{
    "brand": "品牌名称",
    "model_name": "官方商品名称",
    "product_code": "商品编号或null",
    "category": "服装类别",
    "poshmark_category": {{
        "department": "一级分类",
        "category": "二级分类",
...
- poshmark_category 必须从提供的分类列表中选择，不要编造不存在的分类"""


def get_system_prompt(category_tree: str) -> str:
    return SYSTEM_PROMPT_TEMPLATE.replace("{category_tree}", category_tree)


def load_poshmark_categories() -> str:
    """加载 Poshmark 分类树并格式化为文本"""
    try:
        # 尝试多个路径
        paths = [
        ...
        for path in paths:
            ...
            # 如果文件不存在，返回简化版本
            return """Women: Bags, Shoes, Accessories, Dresses, Intimates & Sleepwear, Jackets & Coats, Jeans, Jumpsuits & Rompers, Leggings, Maternity, Pants, Shorts, Skirts, Suits & Blazers, Sweaters, Swimwear, Tops, Vests
Men: Bags, Shoes, Accessories, Jackets & Coats, Jeans, Pants, Shorts, Suits & Blazers, Sweaters, Swimwear, Tops, Vests, Underwear & Socks
Kids, Home, Pets, Electronics"""

        # 格式化为文本
        lines = []
        for dept in tree:
            cats = [c['display'] for c in dept.get('categories', [])]
            lines.append(f"{dept['display']}: {', '.join(cats[:10])}" + (f" ... 等{len(cats)}个" if len(cats) > 10 else ""))
        return "\n".join(lines)
    except Exception as e:
        print(f"[Gemini] 加载分类树失败: {e}")
        return "Women, Men, Kids, Home, Pets, Electronics"
...
class GeminiClothingAnalyzer:
    ...
    def __init__(self, api_key: str = None, model: str = None):
        ...
        self.client = None
        self.category_tree = load_poshmark_categories()
        self._init_client()
    ...
    def analyze(self, image_path: str, max_retries: int = 2, use_search: bool = False) -> Dict[str, Any]:
        ...
        if use_search:
            ...
            system_instruction = get_system_prompt(self.category_tree)
